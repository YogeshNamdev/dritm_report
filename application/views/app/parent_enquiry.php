    ...
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          ...
          <div class="col-sm-2">
          <a  style="margin-left: 90px;" class="btn btn-sm btn-success"  href="<?php echo base_url(); ?>app/home/export_enquiry">Export</a>
          </div>
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
    </div>
    ...
    <section class="content">
      ...
      function get_all_enquiry_details() {
           ...
           function delete_enquiry(enquiry_id) {
              var conf = confirm("Are you sure to delete this enquiry?");
              if (conf == true) {
                  var base_url = '<?php echo base_url(); ?>';
                  $.ajax({
                      type: "POST",
                      dataType: "JSON",
                      url: base_url + "app/home/delete_enquiry",
                      data: {
...
function get_batch(obj)
{
    ...
    var grade_id = $('#sel_class_id').val();
    //console.log(class_id);
        $.ajax({
            type: "POST",
            url: "<?php echo base_url() ;?>app/home/get_all_batches",
            data: {"grade_id": grade_id},
            //contentType: false,
            cache: false,
            //processData: false,
            ...
            success: function(response) {
                    ...
                    if(response.resp_status == true)
                    {
                        $.each(response.result , function( index, row ) {
                          html += "<option value="+row.grade_id+">"+row.grade_time+"</option>";
                        });
                    }
                        $("#sel_grade_id").html(html);
                }
